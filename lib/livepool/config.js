...
config.loadRules = function() {
    ...
    _.each(poolData.children, function(proj) {
        ...
        // fix base dir /
        var projBase = _.contains(['\\', '/'], proj.match.charAt(handler.length - 1)) ? proj.match : proj.match + path.sep;
        // var 
        var handler = [],
            router = [];
...
config.getHandler = function(reqInfo) {
    ...
    if (config.menuAction.disablePool) {
        return;
    }
...
config.getRouter = function(reqInfo) {
    if (config.menuAction.disablePool == 'true') {
        return;
    }
