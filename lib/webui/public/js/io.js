var initSocketIO = function() {
        ...
        if (proxy.length) {
            ...
            Ext.Array.each(res, function(row) {
                ...
                if (gridRow) {
                    ...
                    row.reqBody && gridRow.set('reqBody', unescape(row.reqBody));
    ...
    io.on('config', function(config) {
        ...
        if (config.proxy) {
            $cmp.chkProxy.setChecked(config.proxy);
        }
        ...
        if (config.speedLimit) {
            ...
            $cmp.speedLimit.menu.items.getAt(map[config.speedLimit]).setChecked(true);
        }
        if (config.layout) {
        ...
        if (config.platform != 'darwin') {
            $cmp.speedLimit.setVisible(false);
        }
    });
};
